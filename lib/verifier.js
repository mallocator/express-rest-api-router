 ...
 */
exports.configure = function({ path, method, config}) {
    if (!config || typeof config === 'function') {
        return (req, res, next) => next();
    }
    config.paramOrder = config.paramOrder || this.globalConfiguration.paramOrder;
    config.paramMap = config.paramMap || this.globalConfiguration.paramMap || 'args';
    if (config.params) {
        for (let param in config.params) {
            var parsed = exports.parseParam(config.params[param]);
            parsed.error = parsed.error | config.error || this.globalConfiguration.error;
            parsed.validate = parsed.validate || config.validate || this.globalConfiguration.validate;
            parsed.success = parsed.success || config.success || this.globalConfiguration.success;
            config.params[param] = parsed;
        }
    }
    this.endpoints[path] = this.endpoints[path] || {};
    this.endpoints[path][method.toUpperCase()] = config;
    return exports.verify.bind(this);
};
