...
exports.configure = function(original, method, endpoint, configuration, ...handlers) {
    ...
    configuration.paramMap = configuration.paramMap || this.globalConfiguration.paramMap || 'arguments';
...
exports.parseParam = function(str) {
    if (typeof str == 'string') {
        ...
        var def = exports.parseValue(type, match[3]);
        return { type, default: def, required }
    }
 ...
 * @returns {*}
 */
exports.parseValue = function(type, value) {
    switch (type) {
        case 'string':
        ...
        case 'bool': case 'boolean':
            switch (value) {
                case 1: case 'TRUE': case 'true': case 'T': case 't': case 'yes': case 'y':
                    return true;
                case 0: case 'FALSE': case 'false': case 'F': case 'f': case 'no': case 'n':
                    return false;
                default:
                    return undefined;
            }
...
exports.verify = function(req, res, next) {
    ...
    if (Object.keys(missing).length) {
        ...
        // TODO support different output formats (json, xml (SOAP?), tree (text), table (text), csv (text))
        ...
        if (req[prop]) {
            for (let param in req[prop]) {
                if (params[param] !== undefined) {
                    params[param] = exports.parseValue(config.params[param].type, req[prop][param]);
                }
            }
        }
    }
