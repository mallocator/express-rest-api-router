...
exports.respond = function(req, res, payload, status) {
    ...
    switch(format.toLowerCase()) {
        ...
        case 'tree':
            return res.send('<html><body style="white-space: pre">' + JSON.stringify(payload) + '</body></html>').end();
        case 'table':
...
exports.formatCSV = function (payload) {
    ...
    for(let i = 0; i < table.headers.length; i++) {
        table.headers[i].replace(/"/g, '\\"');
        if (table.headers[i].indexOf(',') !== -1) {
            table.headers[i] = '"' + table.headers[i] + '"';
        }
    }
